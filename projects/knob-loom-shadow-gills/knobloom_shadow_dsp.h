...
#ifndef KNOBLOOM_SHADOW_DSP_H_
#define KNOBLOOM_SHADOW_DSP_H_
...
class Instrument {
 public:
  ...
  void Init() {
    ...
    aperture_ = 0.0f;
    activity_blocks_ = 0;
    shadow_flash_blocks_ = 0;
    random_state_ = 0x94d049bbu;
    display_divider_ = 0;
    phase_a_ = 0.0f;
    phase_b_ = 0.25f;
    shadow_inside_ = false;
    shadow_gain_ = 1.0f;
    field_energy_ = 0.0f;
    tick_envelope_ = 0.0f;
    tick_low_ = 0.0f;
    shadow_position_ = 0;
    shadow_event_count_ = 0;
    UpdateTuning();
    UpdateDisplay();
  }
  ...
  void Process(const int32_t controls[PARAM_LAST],
    ...
    static const float pan_left[kNumThreads] = {
      1.00f, 0.96f, 0.84f, 0.66f, 0.45f, 0.26f, 0.12f, 0.05f
    };
    static const float pan_right[kNumThreads] = {
      0.05f, 0.12f, 0.26f, 0.45f, 0.66f, 0.84f, 0.96f, 1.00f
    };
    const float coupling_depth = topology_ == TOPOLOGY_RING ? 0.0022f :
        (topology_ == TOPOLOGY_PAIRS ? 0.0034f : 0.0028f);

    const bool shadow_enabled = aperture_ > 0.015f;
    const float rate_a = 0.22f + 5.78f * tension_ * tension_;
    const float rate_b = rate_a * (1.13f + 0.54f * tension_);
    const float phase_increment_a = rate_a * (1.0f / 48000.0f);
    const float phase_increment_b = rate_b * (1.0f / 48000.0f);
    const float window = 0.012f + 0.50f * aperture_ * aperture_;
    const float inverse_window = 1.0f / window;
    const float shadow_depth = 0.18f + 0.70f * aperture_;
    const float tick_decay = 0.9880f + 0.0105f * aperture_;

    for (size_t sample = 0; sample < BUFSIZE; ++sample) {
      const float noise = RandomBipolar();
      phase_a_ += phase_increment_a;
      phase_b_ += phase_increment_b;
      if (phase_a_ >= 1.0f) phase_a_ -= 1.0f;
      if (phase_b_ >= 1.0f) phase_b_ -= 1.0f;

      const float wave_a = Triangle(phase_a_);
      const float wave_b = Triangle(phase_b_);
      const float distance = fabsf(wave_a - wave_b);
      const bool inside = shadow_enabled && distance < window;
      const float proximity = inside
          ? Clamp01(1.0f - distance * inverse_window)
          : 0.0f;
      if (inside && !shadow_inside_) TriggerShadow(proximity);
      shadow_inside_ = inside;

      const float target_gain = inside
          ? 1.0f - shadow_depth * (0.35f + 0.65f * proximity)
          : 1.0f;
      const float shadow_slew = target_gain < shadow_gain_ ? 0.085f : 0.010f;
      shadow_gain_ += shadow_slew * (target_gain - shadow_gain_);
      ...
      const float field_left = sum_left * 0.12f;
      const float field_right = sum_right * 0.12f;
      const float field_level = Clamp(
          (fabsf(field_left) + fabsf(field_right)) * 1.8f, 0.0f, 1.0f);
      field_energy_ += 0.0025f * (field_level - field_energy_);

      tick_low_ += 0.14f * (noise - tick_low_);
      const float paper = noise - tick_low_;
      const float tick = paper * tick_envelope_ * 0.42f;
      tick_envelope_ *= tick_decay;
      if (tick_envelope_ < 0.000001f) tick_envelope_ = 0.0f;

      const size_t tick_position = shadow_position_;
      const float mixed_left = field_left * shadow_gain_ +
          tick * pan_left[tick_position];
      const float mixed_right = field_right * shadow_gain_ +
          tick * pan_right[tick_position];
      output_left[sample] = static_cast<int32_t>(
          SoftLimit(mixed_left) * 120000000.0f);
      output_right[sample] = static_cast<int32_t>(
          SoftLimit(mixed_right) * 120000000.0f);
    }

    if (activity_blocks_ > 0) --activity_blocks_;
    if (shadow_flash_blocks_ > 0) --shadow_flash_blocks_;
  ...
  bool shadow_enabled() const { return aperture_ > 0.015f; }
  bool shadow_pulse() const { return shadow_flash_blocks_ > 0; }
  uint8_t topology() const { return topology_; }
  uint8_t scale() const { return scale_; }
  int32_t decay_step() const { return decay_step_; }
  float tension() const { return tension_; }
  float aperture() const { return aperture_; }
  float shadow_gain() const { return shadow_gain_; }
  uint32_t shadow_event_count() const { return shadow_event_count_; }
 ...
 private:
  ...
  void UpdateControls(const int32_t controls[PARAM_LAST], int32_t decay) {
    ...
    const float tension_target = Q27ToFloat(controls[PARAM_TENSION]);
    const float aperture_target = Q27ToFloat(controls[PARAM_APERTURE]);
    if (!controls_initialized_) {
      tension_ = tension_target;
      aperture_ = aperture_target;
    } else {
      tension_ += 0.08f * (tension_target - tension_);
      aperture_ += 0.08f * (aperture_target - aperture_);
    }
  ...
  size_t CouplingSource(size_t thread) const {
    ...
    if (topology_ == TOPOLOGY_STAR) {
      if (thread == 3) return reverse_ ? 7 : 0;
      if (thread == 4) return reverse_ ? 0 : 7;
      return thread < 4 ? 3 : 4;
    }
    if (topology_ == TOPOLOGY_PAIRS) {
      return thread ^ 1u;
    }
    return (thread + 4) % kNumThreads;
  }

  size_t ShadowPositionForEvent(uint32_t event) const {
    static const uint8_t ring_sequence[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    static const uint8_t star_sequence[8] = {3, 0, 3, 2, 4, 5, 4, 7};
    static const uint8_t pair_sequence[8] = {0, 1, 2, 3, 4, 5, 6, 7};
    static const uint8_t cross_sequence[8] = {0, 4, 1, 5, 2, 6, 3, 7};
    const uint8_t* sequence = ring_sequence;
    if (topology_ == TOPOLOGY_STAR) sequence = star_sequence;
    if (topology_ == TOPOLOGY_PAIRS) sequence = pair_sequence;
    if (topology_ == TOPOLOGY_CROSS) sequence = cross_sequence;
    size_t step = static_cast<size_t>(event & 7u);
    if (reverse_) step = 7u - step;
    return sequence[step];
  }

  void TriggerShadow(float proximity) {
    shadow_position_ = static_cast<uint8_t>(
        ShadowPositionForEvent(shadow_event_count_));
    ++shadow_event_count_;
    const float energy = Clamp(
        field_energy_ * (0.45f + 1.05f * aperture_) *
        (0.65f + 0.35f * proximity),
        0.0f, 0.24f);
    tick_envelope_ = std::max(tick_envelope_, energy);
    shadow_flash_blocks_ = 80;
    activity_blocks_ = std::max<uint16_t>(activity_blocks_, 80);
  }
  ...
  void UpdateDisplay() {
    ...
    Put(line3_, 16, "T");
    Put2(line3_, 17, static_cast<int32_t>(tension_ * 99.0f + 0.5f));
    Put(line4_, 0, reverse_ ? "REV" : "FWD");
    Put(line4_, 4, topology_name[topology_]);
    Put(line4_, 9, "A");
    Put2(line4_, 10, static_cast<int32_t>(aperture_ * 99.0f + 0.5f));
    Put(line4_, 13, shadow_enabled() ? "SHADOW" : "CLEAR");
  }
  ...
  float shadow_gain_;
  float field_energy_;
  float tick_envelope_;
  float tick_low_;
  uint8_t shadow_position_;
  uint32_t shadow_event_count_;
  char line1_[22];
  char line2_[22];
  char line3_[22];
  char line4_[22];
